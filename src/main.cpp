...
#define start_sample_sent_attempt 5 //amount of measurements where the sent attempts are starting
...
void setup()
{
  ...
  while (1)
  {
    ...
    {
    case systemState_unknown:   //first run/sensors not connected
    ...
    case systemState_readMeasurement:   //read measurements
    {
      ...
      if ((temperature_sensor1.getTempCByIndex(0)) == -127) //REVIEW, this fault should be fetched on a better way
      {
#if debugMessages
      ...
      //printf("measurement[%u].temperature1: %f\n", currentMeasurement, measurement[currentMeasurement].temperature1);
      ...
      else if (currentMeasurement >= maximum_samples_memory) //if the maximum memory space is reached
      {
#if debugMessages
      ...
      else if (currentMeasurement >= start_sample_sent_attempt) //if there are enough samples to sent and sent action has succeed
      {
        sent_ESPNOW_message();
      }
      else if ((currentMeasurement >= start_sample_sent_attempt)) //if there are enough samples to sent and sent action has failed
      {
#if debugMessages
