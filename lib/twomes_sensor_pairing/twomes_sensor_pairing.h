...
void onDataReceive(const uint8_t *macAddress, const uint8_t *payload, int length) {
  ...
  esp_err_t err;
  nvs_handle gatewayHandle;
  ESP_LOGD(TAG, "Received a pairing message!");
  //Open the NVS partition:
  ...
  if (err == ESP_OK) {
    ...
    ESP_LOGD(TAG, "stored MAC address: %02X:%02X:%02X:%02X:%02X:%02X, and Channel %u", macAddress[0], macAddress[1], macAddress[2], macAddress[3], macAddress[4], macAddress[5], *payload);
    ...
    ESP_LOGD(TAG, "Stored Pairing data");
